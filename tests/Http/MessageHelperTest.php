...
class MessageHelperTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testGetParsedBodyMap1()
    {
        $items = ['foo' => 'bar'];
        $request = new \Zend\Diactoros\ServerRequest();
        $request = $request->withParsedBody($items);
        $this->assertEquals(new \HH\Map($items), $this->getParsedBodyMap($request));
    }

    public function testGetParsedBodyMap2()
    {
        $request = new \Zend\Diactoros\ServerRequest();
        $this->assertEquals(new \HH\Map(), $this->getParsedBodyMap($request));
    }
    ...
    {
        $this->assertInstanceOf(\Caridea\Http\PaginationFactory::class, $this->paginationFactory());
    }
}
